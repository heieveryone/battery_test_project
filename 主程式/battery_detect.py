...
PDS20_36A = instrument.power_supply("ASRL18::INSTR", "PDS20")
time.sleep(0.5)
...
# —— 充電停止門檻常數 —— #
charge_VOLtage_LOW, charge_VOLtage_HIGH = 4.1, 4.11
charge_CURRENT_THRESH = 0.052
# —— 放電停止門檻常數 —— #
discharge_voltage_LOW, discharge_voltage_HIGH = 3.8, 3.81
# —— 充放電循環計數器 —— #
...
def cycle_thread():
    ...
    global mode, df_101, df_102, df_111, instrument_flags
    while not shutdown.is_set():
        try:
        ...
        try:
            ...
            # 停止 DAQ 掃描
            DAQ_970a.scan_stop()
            instrument_flags['psu'] = False
            instrument_flags['load'] = False
            instrument_flags['daq'] = False
            print("已關閉 PSU、電子負載 及 DAQ")
        except Exception as e:
            print("停用裝置時發生錯誤：", e)
        time.sleep(60)  # 休息 10 min
        print("== 1min 休息结束，開始 EIS 量測 ==")
        instrument_flags['lcr'] = True
        LCR_EIS_sweep(
            LCR_ZM2371,
            start_freq  = -1,
            stop_freq   =  5,
        ...
        print("== EIS 完成，休息 1min ==")
        instrument_flags['lcr'] = False
        time.sleep(60)
...
def DAQ():
    ...
    global latest_voltage, latest_current, latest_temperature, instrument_flags
    while not shutdown.is_set():
        DAQ_ON_event.wait()
        DAQ_ON_event.clear()
        if shutdown.is_set():
...
def LCR_EIS_sweep(lcr, start_freq, stop_freq, points):
    ...
    for freq in freqs:
        ...
        if len(split_result) == 3:
            real = float(split_result[1])
            imag = float(split_result[2])
            latest_eis = (float(LCR_response_freq), real, imag)
