...
# —— 充電停止門檻常數 —— #
charge_VOLtage_LOW, charge_VOLtage_HIGH = 3.94, 3.95
...
def PSU():
    if charge_or_discharge_flag.get() == "charge":
        try:
            volt, curr = PDS20_36A.output_Set(4.2, 3)
            print("PSU set")
            time.sleep(0.5)
...
if __name__ == "__main__":
    ...
    cycle_t = threading.Thread(target=cycle, daemon=True)
    plc_t.start()
    cycle_t.start()
    ...
    cycle_t.join(timeout=1)
    print("All threads exited, program done.")
"""
PLC_thread = threading.Thread(target = PLC_safe, daemon=True)
PSU_thread = threading.Thread(target = PSU, daemon=True)
